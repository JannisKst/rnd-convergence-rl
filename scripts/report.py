...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.conditions:
        ...
        # t_plateau is itself a function of that setting, so an answer read off one cell
        # of the sweep is the most favourable cell until it is shown not to be.
        tracking_sweep(frame, pin, args.conditions).to_csv(
            args.out / "tracking_sweep.csv", index=False
        )
